...
void
HFLumiGetter::bookHistograms(DQMStore::IBooker & ibooker,
                                  edm::Run const & iRun,
                                  edm::EventSetup const & /* iSetup */)
{
  ...
  if(fHistHFRecordedByLS==0)
    fHistHFRecordedByLS=ibooker.book1D("totalHFRecordedLumiByLS","HF Recorded Lumi vs LS",8000,0.5,8000.5);

  if(fHistHFDeliveredByLS==0)
    fHistHFDeliveredByLS=ibooker.book1D("totalHFDeliveredLumiByLS","HF Delivered Lumi vs LS",8000,0.5,8000.5);

  fHistHFRecordedToDeliveredRatioByLS = ibooker.book1D("HFRecordedToDeliveredRatioByLS","HF Recorded to Delivered Lumi Ratio vs LS",
						     8000,0.5,8000.5);
}

void HFLumiGetter::dqmBeginRun(edm::Run const& run, edm::EventSetup const& es)
{
}
